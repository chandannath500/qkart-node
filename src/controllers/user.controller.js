...
const getUser = catchAsync(async (req, res) => {
  ...
  if(data.email !== req.user.email){
    throw new ApiError(httpStatus.FORBIDDEN,"User not authorized to view other usre's data");
  }
 if(req.query.q === "address"){
  res.send({
...
const getEmail  = catchAsync(async (req, res) => {
  const response = await userService.getUserByEmail(req.params.emailId);
  if(!response){
